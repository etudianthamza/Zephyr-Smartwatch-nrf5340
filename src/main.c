...
static void update_ui_sensors(void)
{
    ...
    // Lire l'heure actuelle du RTC pour l'afficher dans les logs
    rtc_time_t now;
...
int main(void)
{
    ...
    while (1) {
        // Gestion du timer d'acquisition
        if (acq_flag) {
            acq_flag = false;
            sensors_update();
            update_ui_sensors();
